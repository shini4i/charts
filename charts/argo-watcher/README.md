...
Two provider-side settings decide whether it works, and both fail quietly. The
redirect URI must be the application's base URL **including the trailing
slash**. The provider must emit a `groups` claim in its userinfo response under
a scope argo-watcher requests (`profile` or `email`) — without it sign-in
succeeds while the redeploy button and deploy-lock toggle stay hidden. The
upstream [OIDC guide](https://argo-watcher.readthedocs.io/en/latest/guides/oidc/)
